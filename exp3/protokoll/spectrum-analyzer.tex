...
\begin{abstract} %Andi
Verschiedenfarbige Leuchtdioden zeigen in der Absorption ihre gr��te Empfindlichkeit in einem leicht zur Emissionswellenl�nge blauverschobenen Bereich. Ziel des Projekts war es nun, diesen in einer Vormessung nachgewiesenen Effekt zu nutzen, um ein Spektrometer aus verschiedenfarbigen LEDs zu konstruieren. 
Dabei wurden zun�chst die Absorptionskurven verschiedener Leuchtdioden mit Hilfe einer Halogenlampe bekannten Spektrums ermittelt. Anschlie�end diente eine Anordung von 16 dieser LEDs als eigentliches Spektrometer. Das Spektrum einer unbekannten Lichtquelle im optischen Bereich konnte dann mit einer eigens entwickelten Auswertesoftware durch Entfaltung der Absorptionskurven ermittelt werden. Am Ende gelang es so, ein Spektrometer zu entwickeln, das vor allem bei Quellen mit schwarzk�rper�hnlicher Abstrahlcharakteristik gute Resultate liefert. Gegenw�rtig wird das Ger�t unter Verwendung kleinerer SMD-Bauteile neu konstruiert und optimiert. 
\end{abstract}
\newpage

\section{Einleitung} %Andi

Spektrometer sind sind in verschiedensten Bereichen der Physik von �berragender Bedeutung. Die Astronomie hat der Spektralanalyse weite Teile ihrer erzielten Durchbr�che zu verdanken und die Bedeutung von Spektren in der Optik muss nicht gesondert hervorgehoben werden. So finden heute  Spektrometer etwa im James Webb Space Telescope oder in der R�ntgenastronomie Anwendung. Im Projekt war es nun das Ziel, ein solches f�r die Physik enorm bedeutsames Ger�t zu konstruieren. Die Idee, dies unter Ausnutzung der unterschiedlichen Absorptionseigenschaften von verschiedenfarbigen Leuchtdioden zu versuchen, erschien schon aus einem sehr pragmatischen Grund vorteilhaft: Kommerziell w�re ein solches Ger�t verh�ltnism��ig kosteng�nstig herzustellen, auch wenn der im Projekt verwendete Versuchsaufbau wohl dem Gegenwert eines Kleinwagens entspricht.
...
\end{center}
\vspace{-1.5\baselineskip}
\caption{Elektronik zum Messen der Photströme der LEDs}
