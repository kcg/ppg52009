...
filenames = []; pfad_rohspektren = '2010-01-11'
...
# Offset entfernen
# Wähle hierzu das zehntkleinste Element
for s in spektren:
	s1 = sc.array(s[1])
	s1.sort()
	s[1] = s[1] - s1[10]
...
leucht_data = readdata("kalibrationsspektrum/halogen.dat")
lambda_leucht = sc.array([i[0] for i in leucht_data])
leucht_I = sc.array([i[1] for i in leucht_data])
...
print "Spektren mit der Halogenlampe skalieren ..."
spektren_fertig = []
for i in range(len(spektren_d)):
	spektrum = []
...
pl.savefig("spektren_mit_halogen.pdf")
...
for i in range(len(spektren_fertig)):
	...
	ofile = open("spektren_reduziert2" + "/" + filenames[i], "w")
